...
### Pre-exec authorization check — deny fast, don't waste an exec call

Before calling `exec` for any **write** operation (transfer, reorder, UPDATE):
1. Call `exec` with the write command and `--telegram-id`. The API will enforce role and store-scope via PostgreSQL RBAC.
2. If the response is HTTP 403 → explain what was blocked and what the user can do instead.
3. Do NOT attempt a pre-flight identity lookup before the write.
